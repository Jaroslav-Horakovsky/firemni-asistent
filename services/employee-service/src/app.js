...
app.get('/health', async (req, res) => {
  try {
    ...
    try {
      await secretsManager.getSecret('DB_EMPLOYEE_SERVICE_URL')
      secretsHealthy = true
    } catch (error) {
...
async function startServer() {
  try {
    console.log('[Server] Starting employee-service...')
    ...
    await database.createEmployeeTables()
    ...
    const server = app.listen(PORT, () => {
      console.log('[Server] Employee service ready on port', PORT)
      console.log('[Server] Health check: http://localhost:' + PORT + '/health')
      console.log('[Server] Database: Connected with all tables created')
      console.log('[Server] RELACE 30: Employee Service foundation complete!')
    })
